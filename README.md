...
generated script depends on nothing but pyguitest — and it needs 0.9.0 or newer
to have them. They name what was wrong instead of raising a bare
...
**pyguitest 0.9.0 or newer is required outright.** Generated scripts call the
`expect_` family and `double_click_element` as pyguitest `Session` methods, which
do not exist before 0.9.0 — and too old a pyguitest does not fail loudly, because
the generator asks the installed `Session` which methods exist before it emits a
call to one, so an older version quietly produces a weaker script instead.
`--doctor` reports whether the installed pyguitest is new enough.
